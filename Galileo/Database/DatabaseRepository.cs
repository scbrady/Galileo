...
GROUP BY c.course_id, c.course_name, c.course_submit_day, c.course_date_created, c.course_begin_date, c.course_end_date, c.course_is_enabled, c.course_ref_grade, c.course_ref_hours;";
        ...
        {
            string sql = @"
select p.*, sum(e.entry_total_time) as project_total_time,
...
group by p.project_begin_date, p.project_course_id, p.project_created_by, p.project_date_created, p.project_description, p.project_end_date, p.project_id, p.project_id, p.project_is_enabled, p.project_name;";
        ...
        {
            string sql = @"
SELECT u.user_id, u.user_first_name, u.user_last_name, sum(e.entry_total_time) as user_total_time, c.course_begin_date as user_begin_date, c.course_end_date as user_end_date
from [USER] u
...
order by user_id";
        ...
        {
            string sql = @"
SELECT user_id, user_first_name, user_last_name, SUM(entry_total_time) as user_total_time, p.project_begin_date as user_begin_date, p.project_end_date as user_end_date
...
order by user_total_hours";
        ...
        {
            string sql = @"
    SELECT user_id, r.team_id, user_first_name, user_last_name, ISNULL(SUM(entry_total_time)/60, 0) as user_total_time, p.project_begin_date as user_begin_date, p.project_end_date as user_end_date,
...
WHERE c.course_id = @courseId AND ru.student_id is NULL AND UPPER(m.member_position) != UPPER('Teacher')";
        ...
        {
            string sql = @"
SELECT DISTINCT(u.user_id), u.user_first_name, u.user_last_name, MAX(ru.team_id) as project_id,
